...
private:
    ...
    // ==== additional private helpers for renderMaze  ====

    // changes the pixel at location loc to red 
    void setRed(PNG& im, pair<int,int> loc);

    // builds a vector containing the locations of the 
    // 49 pixel locations for a 7px by 7px square 
    // where curr is at the center 
    // does not pay any attention to whether or not the neighbors are 
    // valid (in the image).
    Queue<pair<int, int>> startSquare(pair<int,int> curr);
